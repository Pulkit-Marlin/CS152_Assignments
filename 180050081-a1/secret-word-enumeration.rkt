...
(define (check-complete-key l key ans) ;;l is list of possible list if first 6 - returns either #f or key(when multiple) or complete-key
  ...
  (cond [(> (length ans) 1) (begin (displayln "swe: multiple matches found") key)]
        [(null? l) (cond [(null? ans) #f]
                         [(= (length ans) 1) (begin (displayln "swe: complete key found!")
                                                    (display "swe: secret word is ")
                                                    (match (car ans) [(list a b c d e f g ... ) (displayln (list->string (list a b c d e f)))])
                                                    (append* ans))]
                         [else (begin (displayln "swe: multiple matches found") key)])]
        [else (let* ([l1 (utils:encryption-key (list->string (car l)))]
                     [v (valid key l1)])
                              (if v (begin (display "swe: potential consistent candidates: ")
                                           (displayln (car l))
                                           (check-complete-key (cdr l) key (append (list l1) ans)))
                                 (check-complete-key (cdr l) key ans)))]))
;                (if v (if (check-in-dictionary (list->string (car l))) (begin (display "swe: potential consistent candidates: ")
;                                                                              (displayln (car l))
;                                                                              (check-complete-key (cdr l) key (append (list l1) ans)))
;                          (check-complete-key (cdr l) key ans))
;                    (check-complete-key (cdr l) key ans)))]))

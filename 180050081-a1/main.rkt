...
(require racket/list
         ...
         (prefix-in  algo: "secret-word-enumeration-1.rkt"))
...
(define (crack-cipher strategies key) ;; Returns list of encryption keys.
  ...
  (crack-cipher-helper strategies key '()))

(define (crack-cipher-helper strategies key ans)
  

(define (crack-cipher-helper2 strategy key)   ;;returns list of keys
  (displayln "\n NEW STRATEGY \n")
  (crack-cipher-helper3 (strategy key) key '()))

(define (crack-cipher-helper3 subs-list key ans)
  (if (null? subs-list) ans
      (crack-cipher-helper3 (cdr subs-list) key (let ([x (check (car subs-list) key)])
                                                  (if (not x) ans
                                                      (append ans (list x)))
                                                  ))))


(define (check substi key)
  (displayln "\n NEW SUBSTITUTION \n" )
  (let ([dc (algo:dictionary-closure (utils:add-substitution substi key))])
    (if (not dc) #f
        (let ([swe (algo:secret-word-enumeration dc)])
          (cond [(not swe) #f]
                [(member #\_ swe) (begin (display "@@@+ ") (displayln index) (utils:show-key swe) (displayln "@@@-") (set! index (+ 1 index)) #f)]
                [else (if (member swe ans) (begin (displayln "@@@+ ") (utils:show-key swe) (displayln "@@@-") (set! index (+ 1 index)) swe)
                                                   (begin (display "@@@+ ") (displayln index) (utils:show-key swe) (displayln "@@@-") (set! index (+ 1 index)) swe))])))))
;          (if (or (not swe) (member #\_ swe)) #f
;              (begin (displayln "\n") (utils:show-key swe) swe))))))
;

  (if (null? strategies) 
      (if (null? ans) (let ([my-check2 (algo:secret-word-enumeration-1 key)])
                        (if my-check2
                            (if (member #\_ my-check2)
                                ans (begin
                                      (displayln "\n AS ALL THE SUBSTITUTIONS GIVEN BY STRATEGIES DON'T COMPLETE KEY, MAYBE MY STRATEGY FAILS IN THIS CASE, \n SO I DIRECTLY CHECK FOR SWE-1 WHICH CHECKS WHETHER EVERY WORD IN CIPHERTEXT FORMS A MEANINGFUL WORD WITH THE SECRET WORD \n")
                                      (list my-check2))) ans)) (remove-duplicates  ans))
      (crack-cipher-helper (cdr strategies) key (let ([x (crack-cipher-helper2 (car strategies) key)]) (append (remove* x ans) x)))))
